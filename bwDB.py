...
# modified for Python 3.5 by David Pondelick
...
def test():
    ...
    db.sql_do(' CREATE TABLE {} ( id INTEGER PRIMARY KEY, string TEXT, number INTEGER ) '.format(t))
    print('Done.')
    ...
    print('There are {db.countrecs()} rows')
    ...
    print('There are {} rows'.format(db.countrecs()))
    print('Now delete it')
    db.delete(newid)
    print('There are {} rows'.format(db.countrecs()))
    for r in db.getrecs():
        print(dict(r))
    for r in db.sql_query('select * from {}'.format(t)):
        print(r)
    for r in db.sql_query('select * from {}'.format(t)):
        print(list(r))
    for r in db.sql_query('select * from {}'.format(t)):
